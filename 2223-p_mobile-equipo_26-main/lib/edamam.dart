...
const TYPE = "public";
const APP_ID = "8ff3823c";
const APP_KEY = "46c5c77148e705f8397c06c677de7084";
...
class Recipe {
  ...
  String toString() {
    return "\tServings: $servings\n\n" +
        ...
        "\tGlycemic Index: ${glycemicIndex?.toStringAsFixed(2)}\n\n" +
        "\tTotal CO2 Emissions: ${totalCO2Emissions?.toStringAsFixed(2)}\n\n" +
        "\tCo2 Emissions Class: $co2EmissionsClass\n\n" +
...
Future<RecipeBlock?> search_recipes(String query, String diet, String cuisineType, String mealType) async {
  // TODO: include a search criteria!
  var formattedQuery =
      "type=$TYPE&beta=true&app_id=$APP_ID&app_key=$APP_KEY";
  if(query != ""){
    formattedQuery = "$formattedQuery&q=$query";
  }
  if(diet != ""){
    formattedQuery = "$formattedQuery&diet=$diet";
  }
  if(cuisineType != ""){
    formattedQuery = "$formattedQuery&cuisineType=$cuisineType";
  }
  if(mealType != ""){
    formattedQuery = "$formattedQuery&mealType=$mealType";
  }
...
void main(List<String> arguments) async {
  if (arguments.isEmpty) {
    ...
    try {
      var block = await search_recipes(arguments[0], arguments[1], arguments[2],arguments[3]);
      print(block);
    } catch (exception) {
      print(exception);
    }
